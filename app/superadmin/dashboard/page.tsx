...
export default function SuperAdminDashboard() {
    ...
    const loadAll = async () => {
        ...
        const [{ data: shopsData }, { data: usersData }, { data: checkins }] =
            await Promise.all([
        ...
        // Get exact customer counts per shop using count queries
        const now = new Date()
        ...
        await Promise.all(
            (shopsData || []).map(async (shop) => {
                ...
                statsMap[shop.id] = {
                    ...
                    checkinLast30: shopCheckins.filter(
    ...
    const createShop = async () => {
        ...
        // Insert shop
        const slug = newShop.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')

        const { data: shopData, error: shopErr } = await supabase
            .from('shops')
            .insert({ name: newShop.name.trim(), slug })
            .select()
            .single()
